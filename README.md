...
- Download and extract `afk_monitor_standalone.7z` to a folder
...
- On the AFK Monitor GitHub page use Code > Download ZIP and extract the contents to a new folder
- Copy `afk_monitor.example.toml` and rename the copy to `afk_monitor.toml`
- (Optional) For Discord support edit `afk_monitor.toml` (instructions in file)
